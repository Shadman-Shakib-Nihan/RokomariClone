...
const form = useForm({
    attribute_id: String(props.option.attribute_id),
    value: props.option.value,
    sort_order: String(props.option.sort_order ?? ''),
    color_hex: props.option.color_hex ?? '',
});

function submit() {
    form.put(admin.attributeOptions.update.url(props.option.id), {
        preserveScroll: true,
        onSuccess: () => {
...
<template>
    ...
    <AppLayout
        :breadcrumbs="[
            ...
            { title: 'Options', href: '#' },
            { title: 'Edit', href: admin.attributeOptions.edit.url(option.id) },
        ]"
    >
        <div class="flex h-full flex-1 flex-col gap-4 p-4">
            <Heading
                :title="`Edit: ${option.value}`"
                description="Update option details"
            />
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        ...
                        <div class="flex items-center gap-2">
                            <Button type="submit" :disabled="form.processing">
                                {{ form.processing ? 'Updating...' : 'Update Option' }}
                            </Button>
                        </div>
                    </form>
                </CardContent>
