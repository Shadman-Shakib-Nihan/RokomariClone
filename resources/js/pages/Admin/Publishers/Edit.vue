...
const existingLogoUrl = computed(() => {
    if (!props.publisher.logo) return null;
    if (props.publisher.logo.startsWith('http')) return props.publisher.logo;
    return `/storage/${props.publisher.logo}`;
});
...
function submit() {
    form.put(admin.publishers.update.url(props.publisher.id), {
        preserveScroll: true,
        onSuccess: () => {
...
<template>
    ...
    <AppLayout
        :breadcrumbs="[
            ...
            { title: 'Edit', href: admin.publishers.edit.url(publisher.id) },
        ]"
    >
                    ...
                    <form @submit.prevent="submit" class="space-y-6">
                        <div class="grid gap-4 md:grid-cols-2">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center gap-4">
                                    <div
                                        v-if="previewUrl || (existingLogoUrl && !form.logo)"
                        ...
                        <div class="flex items-center gap-2">
                            <Button type="submit" :disabled="form.processing">
                                {{ form.processing ? 'Updating...' : 'Update Publisher' }}
                            </Button>
                        </div>
                    </form>
                </CardContent>
